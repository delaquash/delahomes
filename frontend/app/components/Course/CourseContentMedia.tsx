...
import { useAddNewAnswerMutation, useAddNewQuestionMutation, useAddReplyToReviewMutation, useAddReviewInCourseMutation, useGetAllCourseQuery } from '@/redux/features/course/coursesApi';
...
const CourseContentMedia = ({data, activeVideo, setActiveVideo, user, refetch, id}: Props) => {
    ...
    const {data:course} = useGetAllCourseQuery(id);
    ...
    const isReviewExist = data?.reviews?.find((review: any)=>review.user._id === user._id);
      ...
      },[isSuccess, error, answerSuccess,answerError, reviewError, reviewSuccess,replySuccess, replyError])
        ...
        <div className="w-full">
          <>
            {!isReviewExist && (
              <>
                <div className="flex w-full">
                  <Image src={user.avatar? user.avatar.url : "avatarImage.jpg"} width={50} height={50} alt="" className="w-[50px] h-[50px] rounded-full object-cover"/>
                  <div className="w-full">
                  ...
                  return (
                    <div className="w-full my-5 dark:text-white text-black" key={index}>
                      <div className="w-full flex">
                        <div>
                          <Image src={item.user.avatar ? item.user.avatar.url : "avatarImage.jpg"}
                      ...
                      {item.commentReplies.map((i: any, index: number) => (
                        <div className="w-full flex 800px:ml-16 my-5" key={index}>
                          <div className="w-[50px] h-[50px]">
                            <Image src={i.user.avatar ? i.user.avatar.url : "avatarImage.jpg"}
                              width={50} height={50} alt="" className="w-[50px] h-[50px] rounded-full object-cover"/>
                          </div>
                          <div className="pl-2">
          ...
          <>
            {item.questionReplies.map((item: any, i:any) => (
              <div className="w-full flex 800px:ml-16 my-5 text-black dark:text-white" key={i}>
                <div>
                  <Image src={item.user.avatar ? item.user.avatar.url : "avatarImage.jpg"} width={50} height={50}
